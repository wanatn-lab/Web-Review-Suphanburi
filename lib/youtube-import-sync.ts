...
/**
 * Imports recent public Shorts candidates into the private review queue.
 * Both Vercel Cron and the authenticated admin page use this function, so
 * manual sync follows exactly the same deduplication and moderation rules.
 */
export async function syncYouTubeImports(): Promise<YouTubeSyncResult> {
  ...
  return {
    ...
    message: "Shorts candidates are private until an editor publishes them from the admin queue.",
  };
}
