                                                            ...
                                                            - - ไม่ได้แก้ `package.json` หรือ `package-lock.json`
                                                              - - ไม่ได้สร้างหรือแก้ไฟล์ `.env` ใดๆ
                                                                ...
                                                                - 1. ตรวจ PR นี้ (diff เฉพาะไฟล์ที่ระบุข้างบน + `PROGRESS.md`)
                                                                  2. 2. Merge เข้า `main`
                                                                     3. 3. Apply migration `supabase/006_add_facebook_tokens.sql` ผ่าน Supabase SQL Editor (เหมือนที่เคยทำกับ migration 005)
                                                                        4. 4. เพิ่ม `FB_APP_ID` และ `FB_APP_SECRET` ใน Vercel Environment Variables (ดูวิธีหาค่าด้านบน)
                                                                           5. 5. Redeploy
                                                                              6. 6. ทดสอบยิง `/api/refresh-facebook-token` ด้วย `Authorization: Bearer <CRON_SECRET>` เองสัก 1 ครั้ง เพื่อยืนยันว่า exchange กับ Facebook สำเร็จและมีแถวใหม่ในตาราง `facebook_tokens`
                                                                                 7. 7. ทดสอบ `/api/sync-facebook` อีกครั้งหลังจากนั้น เพื่อยืนยันว่ายังอ่าน token ได้ปกติ (คราวนี้จาก Supabase แทน env var โดยตรง)
                                                                                   
                                                                                    8. ## Environment variables ที่ระบบใช้งาน
                                                                                   
                                                                                    9. ระบุเฉพาะชื่อ ไม่บันทึกค่าจริง:
