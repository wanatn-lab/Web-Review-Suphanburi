...
export async function setMustVisit(reviewId: string, pinned: boolean) {
  ...
  const { error } = await getSupabaseAdmin().rpc("set_must_visit", { target_review_id: reviewId, should_pin: pinned });
  if (error) throw new Error(error.message);
  revalidateMustVisit();
}
export async function saveMustVisitOrder(ids: string[]) {
  assertAdmin();
  if (!Array.isArray(ids) || ids.length > 100 || new Set(ids).size !== ids.length || ids.some((id) => !/^[0-9a-f-]{36}$/i.test(id))) throw new Error("Invalid order");
