...
export interface Review {
  ...
  tiktok_embed_url: string | null; google_map_embed_url: string | null; latitude: number | null;
  longitude: number | null; location_text: string | null; facebook_post_id: string | null;
  created_at: string; deleted_at: string | null;
}
type ReviewRow = Omit<Review, "category_label"> & { categories: { label: string } | { label: string }[] | null; };
const REVIEW_COLUMNS = "id, title, slug, description, category, cover_image, facebook_embed_url, tiktok_embed_url, google_map_embed_url, latitude, longitude, location_text, facebook_post_id, created_at, deleted_at, categories(label)";
