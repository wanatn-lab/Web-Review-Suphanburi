...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const configuredDescription = category.seo_description?.trim();
  const intro = configuredDescription && [...configuredDescription].length >= 80
    ? configuredDescription
    : getCategorySeoIntro(category.slug, label);
  const description =
    intro ||
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const structuredData = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        mainEntity: {
          "@type": "ItemList",
          name: `รายการรีวิว${label}สุพรรณบุรี`,
  ...
  return (
    <main className="px-4 py-8 sm:px-8">
      ...
      <p className="mb-6 max-w-3xl text-sm leading-7 text-neutral-600 dark:text-neutral-300">{intro}</p>
