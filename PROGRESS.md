...
- เพิ่มหน้า `/admin/manual-content` สำหรับเพิ่มรีวิวด้วยมือ โดยใช้ Supabase Auth แบบ Magic Link: ผู้ดูแลกรอกอีเมลและยืนยันจากลิงก์ครั้งเดียว โดยอนุญาตเฉพาะอีเมลใน `ADMIN_EMAILS`
- เพิ่ม `/auth/callback` เพื่อรับลิงก์ยืนยัน, แลกเป็น session และตรวจสอบอีเมลอีกครั้งก่อนเปิดหน้าผู้ดูแล; middleware จะต่ออายุ session เฉพาะเส้นทาง `/admin`
...
- เพิ่มโหมดวางแคปชั่นด้วยตัวเอง เพื่อสร้างร่าง SEO/GEO ได้แม้ Facebook ยังไม่อนุญาตให้ token อ่านโพสต์ และเก็บลิงก์ TikTok ลงช่อง embed ที่ถูกต้อง
- เพิ่มการวางลิงก์ TikTok เพื่อดึงข้อความและรูปหน้าปกผ่าน TikTok oEmbed เป็นฉบับร่าง โดยรองรับเฉพาะวิดีโอสาธารณะที่ TikTok อนุญาตให้ embed
- แก้ validation ให้รับโดเมนรูปหน้าปก `tiktokcdn-us.com` ที่ TikTok oEmbed ส่งกลับมา เพื่อให้บันทึกฉบับร่างจาก TikTok ได้
...
- ต้องตั้ง `ADMIN_EMAILS` ใน Vercel Environment Variables เป็นอีเมลผู้ดูแลที่อนุญาต คั่นหลายอีเมลด้วย comma เช่น `owner@example.com,editor@example.com`; ห้ามบันทึกข้อมูลลับลง git หรือไฟล์ `.env`
- ต้องตั้ง Supabase Dashboard > Authentication > URL Configuration ให้มี `https://reviewsuphanburi.com/auth/callback` ใน Redirect URLs ก่อนใช้ Magic Link

## ผลการตรวจสอบ (รอบล่าสุด)

- `npx tsc --noEmit`: ผ่าน ไม่มี TypeScript error (ทั้งโปรเจกต์ รวมไฟล์ Magic Link)
- Unit tests (`node:test` ผ่าน `tsx`): **19/19 ผ่าน** (เดิม 7 + ใหม่ 12 — token refresh 9 ตัว, pagination 2 ตัว)
- `npm run build`: ผ่าน (คอมไพล์สำเร็จ และ `/admin/manual-content` กับ `/auth/callback` เป็น dynamic route) — ใช้ค่า Supabase/อีเมล placeholder เฉพาะ process และอนุญาตเครือข่ายสำหรับ Google Fonts; ไม่ได้สร้างไฟล์ `.env` หรือแก้ `app/layout.tsx`
- `git diff --check`: ผ่าน
- Secret scan บน diff ทั้งหมด (pattern `EAA…`, `sk_live`, `AIza…`, `ghp_…`, private key header): ไม่พบ
- เพิ่ม dependency `@supabase/ssr` ใน `package.json` และ `package-lock.json` เพื่อจัดการ Supabase Auth session บน Next.js อย่างปลอดภัย
...
6. ตั้ง `ADMIN_EMAILS` ใน Vercel Environment Variables เป็นอีเมลผู้ดูแลที่ต้องการอนุญาต
7. ที่ Supabase Dashboard > Authentication > URL Configuration เพิ่ม `https://reviewsuphanburi.com/auth/callback` ใน Redirect URLs
8. Redeploy แล้วเปิด `/admin/manual-content` กรอกอีเมลที่อยู่ใน `ADMIN_EMAILS` และกดลิงก์ในอีเมลเพื่อทดสอบ
9. ตรวจว่ารายการใหม่เปิดได้ที่ `/reviews/<slug>` และปรากฏใน `/sitemap.xml`
10. ทดสอบยิง `/api/refresh-facebook-token` ด้วย `Authorization: Bearer <CRON_SECRET>` เองสัก 1 ครั้ง เพื่อยืนยันว่า exchange กับ Facebook สำเร็จและมีแถวใหม่ในตาราง `facebook_tokens`
11. ทดสอบ `/api/sync-facebook` อีกครั้งหลังจากนั้น เพื่อยืนยันว่ายังอ่าน token ได้ปกติ (คราวนี้จาก Supabase แทน env var โดยตรง)
...
- `ADMIN_EMAILS` (ใหม่ — รายชื่ออีเมลที่ได้รับสิทธิ์เข้าสู่หน้า Manual Content Entry)
- `FB_PAGE_ID`
